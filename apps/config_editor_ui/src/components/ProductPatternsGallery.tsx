...
export function ProductPatternsGallery(): JSX.Element {
  ...
  const catalogueRows: CatalogueRow[] = [{ id: 'example', name: t('Оптимизация событий', 'Event optimization'), method: t('Поэтапный Top-N', 'Phased Top-N'), status: t('Готов', 'Ready') }];
  ...
  return <section className="product-patterns" aria-labelledby="product-patterns-title">
    ...
    <Pattern id="choice-card" title={t('Карточки выбора', 'Choice cards')} description={t('Крупный одиночный выбор с названием и пояснением — для методов и приоритетов.', 'Large single-choice cards with a name and explanation for methods and priorities.')}>
      <div className="strategy-choice-grid"><Button variant="primary" aria-pressed="true"><strong>{t('Поэтапный Top-N', 'Phased Top-N')}</strong><small>{t('Полный поэтапный поиск с финальной проверкой.', 'Full phased search with final validation.')}</small></Button><Button variant="secondary" aria-pressed="false"><strong>{t('Комбинированный поиск', 'Combined search')}</strong><small>{t('Типы, кодеки и индексы в одном поиске.', 'Types, codecs, and indexes in one search.')}</small></Button></div>
    </Pattern>
    ...
    <Pattern id="summary-panel" title={t('Закреплённое резюме', 'Pinned summary')} description={t('Ключевые решения формы остаются видимыми без отдельного шага проверки.', 'Key form decisions remain visible without a separate review step.')}>
      <aside className="strategy-summary pattern-summary"><span>{t('Шаблон', 'Template')}</span><strong>{t('Поэтапная оптимизация', 'Phased optimization')}</strong><dl><dt>{t('Метод', 'Method')}</dt><dd>{t('Поэтапный Top-N', 'Phased Top-N')}</dd><dt>{t('Этапов', 'Stages')}</dt><dd>6</dd><dt>{t('Кандидатов', 'Candidates')}</dt><dd>100</dd><dt>{t('Приоритет', 'Priority')}</dt><dd>{t('Сбалансированно', 'Balanced')}</dd></dl></aside>
    </Pattern>
